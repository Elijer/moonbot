...
`Moonbot` is an [online mood tracker](http://moonbot.me/login) that empowers users to log personal data each day, such as energy level and sleep schedule. The user can use the settings page to customize which types of data they choose to track. As the user begins tracking data consistently, that data is interpreted into an area graph.

The front-end is mobile-first and built as a "single-page" react app; navigating to a different page in the site does not require the browser to refresh. When logged in, the user may navigate between three pages that each has unique relationships with the database. This multi-page interface is built on a user authentication system with `login`, `logout`, and `registration` events provided as navigation options.

The backend of Moonbot has been split into two separate areas of functionality. One, in the `backend/astral/api` folder, is the user authentication system. This system uses the Django secret to generate `JWT tokens` for each user account; an access token, and a refresh token. The access token has an expiry date baked into it, but a new access token can be retrieved with a valid refresh token. The user-auth system uses these `JSON Web Tokens` to log users in and check whether the current user account is allowed to perform other interactions in the database, such as reading or writing data.

Meanwhile, the routes defined in the `backend/astral` half of the `API` are comprised of views that update or retrieve data, but *only* when a valid access token is provided in the request. The data provided to `Moonbot` is sensitive and describes private information about a user's behavior and life. It is important to have a system that prevents user data to be accessed by users who did not create it. Because the user account and project secret are embedded within the `JWT token`, it would be difficult to access user data by forging one.

Architecturally, intense consideration was put into maximizing the ease and simplicity of the user's experience. It would have been simpler to provide a single form on the main mood tracker page that records all user data from each area of input when submitted. However, people can be distracted. It is common for a form to be accidentally refreshed before it is fully submitted. Form input interruptions like this can be a frustrating experience. To prevent frustration and the potential loss of input, each input element has a handler that validates the data immediately whenever the value of that input changes. Consequently, any valid data entered into the form is sent to the database immediately. Although it is worth considering how an excess of database requests could be unsustainable or overly expensive at scale, I would make the argument that these demands on the backend are justified. Self-reporting is difficult to do consistently, yet consistency is key to generating the quality of data necessary to identify trends in behavior. Therefore, I think that this aggressive data entry pattern is Moonbot's most crucial feature.

Another relatively invisible yet significant feature that promotes a comfortable and effective user experience is conditional display behavior built into each form input; when valid energy or birth control data is entered, the input for that form is hidden. Instead, a positive message takes its place, reporting that the data has been recorded. This is designed to congratulate the user, hopefully creating a positive feedback system that encourages consistent, daily data tracking. It keeps the user from wondering whether they entered data that day, which is not only vaguely distressing but can *also* lead to extra, unnecessary calls to the database, or even potentially overwriting accurate data with inaccurate data.

Both of these examples demonstrate how data accuracy and a simple, communicative user experience go hand in hand.

If done correctly, I think that features like this, which respond to and reward the most effective user behaviors, make the data tracker feel seen, productive, and even understood. In this vein of design, I have added special messages that appear in response to outlier data input, such as reporting crying above 30 or 50 in a day. For a database or an input form, this is just a number. However, if a person were to *tell* you "I cried over fifty times today" you wouldn't just say "Thank you for providing me with this data, I will now save it securely", you would probably say "Oh god, are you okay?", and hug them. It's unlikely that many users will report crying this much. However, I think it's important to try to bend the functionality of these digital processes to mirror the common courtesy of human behavior. Features like these are important because they create a positive illusion to the user that someone is paying attention to their visit to the app. Hopefully, this encourages them to pay the same amount of attention to themselves. After all, attention to oneself is essentially the goal of a mood tracker application; if the user is not empowered to believe that their input is important, they are less likely to report personal data with care or discover insights from it when it is returned to them in the form of a graph.

The goal of this application is to provide a customizable set of behavior tracking tools and, ideally, identify recurring patterns within that behavior. I hope that graphing out these cycles will empower the user to "forecast" their emotional weather; to see violent storms approaching from a distance or glean comfort in a week of clear skies. The interface and aesthetic of Moonbot are heavily geared towards tracking menstruation-related period cycles in behavior and physiological information: period cycles can be accompanied by a wide range of emotional and physical symptoms that are regularly powerful enough to be disruptive. There are also populations of women who experience these symptoms with a dramatically higher magnitude, such as those with Post-Menstrual Distress Disorder. Symptoms of depression, mania, and body aches can be very disruptive. One of the primary goals of Moonbot is to identify patterns in those symptoms so that their cadence can be better understood and their onset does not take the user by surprise.

However, the `settings` page allows the tracked information to be completely customizable. The fields of information that are tracked are important and diverse, yet not too complex, making the app ideal for users who are interested in a set of low-commitment tools to track information about their state of being every day. For many target users, the app must not be overly demanding; one of the most desirable outcomes of this app is to discover trends in negative states of mind such as depression, apathy, or anxiety. As these mental states can easily transform prior commitments into overwhelming burdens, it is crucial that using the app daily requires the minimum amount of buy-in, commitment, and energy. My goal for Moonbot is to create a powerful data generation tool that has the lowest bar for entry when it comes it consistent, self-reported data. Many other mood trackers exist, but a simple yet simultaneously customizable one does not have many peers.

I believe that the aesthetic of this website reflects and bolsters the purpose of `Moonbot`. The purple color scheme was chosen to feel mysterious, intriguing, yet safe. It was also chosen because of its visual proximity to pink and the related feminine connotations. However, I chose to dial it a couple of hues away from the full-blown feminine associations of pink to encourage the use of `Moonbot` for reasons other than period tracking as well. The moon imagery and language link `Moonbot` to the common cultural association between menstruation and lunar cycles. More generally, lunar cycles are also referenced by the look of this app as a metaphor for repeated cycles of human behavior through time. Meanwhile, outer space as a whole represents a realm of discovery and the unknown. I think that these imageries and associations are likely to set the scene for reflection, self-discovery, and maybe a moment of peace.

I chose to use React as a large part of my front-end development pattern for two reasons.

For one, I wanted the user's experience to benefit from how a single-page design does not need to be refreshed to change pages. Secondly, from an organizational standpoint, I foresaw that the complexity of this application would be well-supported by the component encapsulation that React offers. Storing all of the behavior for each tracker component in a dedicated file with its own props, state and logic was a huge boon to being able to give each component sophisticated yet maintainable code. In addition, there were many opportunities where common functionality could be extracted from multiple components and distilled into a single component, shrinking the codebase by removing redundancies, and making it more readable. Surprisingly, these distillations were mostly in `data context`, and not `HTML rendering`. For example, consolidating re-used `HTTP requests` into the `RequestContext` component was an amazing way to prune the code of the `UserSettings` and `Tracker` components down to a much more readable level, while reducing the likelihood of easy-to-make `HTTP request` typos and errors.

What distinguishes this project from others I have created in this course? The main two features of this project that are unique in comparison to the other projects I have created are the Log (graph) page and the Settings Page.

Like retrieving email or listing auctions, the graph page requests data from the database from all or many documents in a collection. It then visually renders that data for an arbitrary number of documents. However, rendering data to a graph was a new challenge for me because it was distinct in many ways from rendering an array of data to `divs` or a list. There *are* some technical reasons for this. The graphing library I used, Recharts.js, will only render data that has been mapped to a certain structure. To get the labels to display the correct variables, this required the creation of some custom logic in the serializers on the backend as well as a bit of remapping on the frontend of the javascript objects they return.

Much more significantly, however, data that has been mapped to a graph is much more demanding when it comes to normalization. Not only must fields be consistent, but the numerical ranges of the data must be represented with values that can be compared, and are consistently the same type, even across different fields in a model. Or, in my case, abstractions of those fields. For example, the date object must be represented in milliseconds, not a string. However, it cannot be *labeled* in milliseconds -- that would be nearly incomprehensible to the user and I quickly found that it even got in the way of me reading test data during development. Even more challenging was creating a graphable value that represented sleep data in a meaningful way. I recorded the time the user went to sleep and when they woke up the following morning on any given day they provided input.

When parsing this in human terms, it's very straightforward (to someone who speaks the same language as you, is an adult with time-processing schooling, and has the same time-reading conventions as you, which is not super common if you consider everyone on earth). You went to sleep at 10 last night and you woke up at 6 this morning. Given that information, it's very easy to infer that you got eight hours of sleep. Good job. If that's normal for you, you will live long and prosper. But in terms of pure data, I was recording the raw information in four separate strings that meant nothing without some system for interpretation. There are some amazing time interpretation tools available, and I have no complaints about their functionality, only their plentitude. Creating a validated "hours-of-rest" value using the user input from the `SleepInput` component was considerably more complicated than simply rendering out `HTML` elements that showed the information. By doing a small amount of data interpretation *in addition* to data collection, this project possesses an extra data appendage that the other project did not. Those projects may have walked on the legs of data reading and writing, but they lacked the arms of data interpretation. My chosen tools for this extra functionality are primitive, but it was an exciting exploration of data interpretation and awakened an excitement for the possibilities available for rendering user data in the modern tech ecosystem. This feature also struck me as meaningful in its symmetry, as the user is giving something to one interface and receiving something back in another.

The second feature that distinguishes this project in complexity from the others I created in this course is the settings page. Unlike graphing, this was relatively straightforward to build. However, it makes this project the first one I have created in which the user can customize their interface, taking some of the burdens off of me as a developer to pre-empt their usage and preferences. I think of this feature as a meta-feature, since it instructs other components how to behave or, in this case, whether they should exist in the user experience. This value of this feature was so obvious to me that it stressed me out a bit because I felt I needed to create it. While creating the mood tracking components, it was very clear that just the presence of certain inputs would be confusing to many users. The app is not *explicitly* a period-tracking app, but the birth control pill tracker and, perhaps the `cry counter` components sort of lock it into that job description. If I had no choice, I am totally fine with locking it in as a period tracking application. Doing one thing well is better than doing a broad set of things mildly well. However, I was aware that it didn't *have* to be locked into this specific function. To be honest, as a person who doesn't experience periods, I was slightly disappointed that the app I had built was close to, but not quite, helpful to me. It was really exciting to me that I could create a feature that would allow me to personalize the app for my own data-tracking interests, and the possibilities this might afford to everyone. After all, I don't know what kind of data the user wants to track! I can narrow it down with some research, but they might not know either.

Another exciting implication of this "meta-feature" is of continuous development; it provides a structure that is very open to additions. In the future, I can easily create new components that extend the functionality of the app.

I think it's interesting how some apps and programs have "meta-features" and some don't. The homepage of an operating system, for example, has many. They can install entire programs, and you can drag around the shortcuts on a desktop view as you see fit. You can even download images and make them the background of the desktop! In some banking and productivity apps, you can customize your dashboard with the "widgets" you find most useful.

I find this an interesting dimension in software development and the user experiences therein because it makes usage much more fluid. With usage analytics, it can even introduce a self-optimizing positive-feedback process in which the behavior of the users themselves can be used to direct the development of future features by analyzing which ones are used the most and are the most loved. To me, this idea casts a critical shadow on the common expectation that a software developer and/or designer must intuit a close understanding of a user's needs. Take one glance around at the development/UX design industry anytime in the last twenty years and it is apparent it is not new to criticize this paradigm. However, like Freud's thrillingly and baseless conjectures within the discipline of psychology, I think the "build it and they will come" methodology is a sort of ground zero on which modern development seems to constantly look back on. There is the idea of the MVP, there is CI/CD, and entire schools of thought about how to continuously perform user research to produce a product that people want, really need, and enjoy. Any engineering paradigm that places users in the driver's seat must contend with the inescapable paradox; the *user* isn't the one creating the product, and so they must constantly be consulted to move forward. Despite this, it is still better than the less efficient and much more expensive method of building something without ever consulting the user about what it is they want._†_ Therefore, any program that attempts to place any valuable freedom in the hands of the user to design their own experience possesses a potential for satisfaction that might not exist otherwise. Although Moonbot only does this on the simplest level, it is thrilling to have built a feature of this kind for the first time, and it certainly sets Moonbot apart from any application I have built before.

_†_ Except, confusingly, in the rare cases of incredible brilliance and innovation, like the iPhone, Instagram, Pokemon Go, Tom's (the shoes), Airbnb, and the fidget spinner. That initial gamble of creation cannot be measured in terms of user appreciation where the user has never seen anything like it. I see this as yet another piece of evidence that user continuous user research cannot be the only or even best answer to innovation. I am interested in the potential of systems of user *participation*, by which a user can customize their own experience, giving developers much more to observe in terms of choice and the direction a piece of technology might go.
